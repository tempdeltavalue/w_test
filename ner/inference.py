...
# Core Model Logic Imports
from image_clf.models import get_mobilenet_v2, mobilenet_v2_transform 
from transformers import AutoTokenizer, AutoModelForTokenClassification, pipeline
from ner.utils import search_animal_class, sorted_classes # sorted_classes is used globally
...
def extract_and_match_animals(text: str) -> List[Tuple[str, str]]:
    """Performs NER, finds 'B-ANIM' entities, and searches for the best match."""
    # This function uses NLP_PIPELINE and sorted_classes internally.
...
# ======================================================================
# 3. ORCHESTRATOR PIPELINE (Argument 'class_names' removed)
# ======================================================================

def combined_inference_pipeline(text: str, image_dir: str):
    
    # 1. Execute NER and search logic
    matched_animals = extract_and_match_animals(text)
    
    # We must access class_names here for the final output via classify_local_image
    ...
    for query, matched_class in matched_animals:
        ...
        if matched_class:
            ...
            # Prepare next input (image path simulation)
            image_path = os.path.join(image_dir, matched_class, f"sample_{matched_class}.jpg")
            
            if IMAGE_CLASSIFIER:
                # 2. Run image classification (passing sorted_classes internally)
...
def main():
    ...
    # Execute the pipeline (class_names is now handled internally)
    combined_inference_pipeline(args.text, args.image_dir)
